% We build the least-squared linear regression model based on the features extracted from PPCA and polynomial feature map.
...
% The tree has just been folded out to one long vector from the top. PolynomialFeaturesLassoModel
...
TwitterFeatureNames = {'Friends x Tweets,Stock Symbol, All Sentiments' ...
   ...
   'Friends x Tweets,Company Name,NeTotalTweetsCountutral', ...
...
PolynomialFeatures = PolynomialFeatureMap(ExtractedTwitterFeatures, StockFeatures);
[COEFF,SCORE] = ppca(zscore(PolynomialFeatures(1:TrainingDataSize,:)), 4);
PolynomialFeatures = zscore(PolynomialFeatures*COEFF);
...
for j = 1:4
    ...
    for i = (TotalDataSize-TestDataSize+1):TotalDataSize
        ...
        for m=1:3
            ...
            for n=0:1
                ...
                for l=(i-10):i
                    ...
                    for k=1:n
                        TrainingInput = [TrainingInput, PolynomialFeatures((4-k):(l-k-1),:)];
                        ValidationInput = [ValidationInput, PolynomialFeatures((l-k),:)];
                    end;
                    
                    TrainingOutput = StockFeatures(4:(l-1),j);
                    ValidationOutput = StockFeatures(l,j);
                    
                    CrossValidationPrediction = LinearRR(TrainingInput, ValidationInput, TrainingOutput,1^(-10));
        ...
        for k=1:Optimaln
            TrainingInput = [TrainingInput, PolynomialFeatures((4-k):(i-k-1),:)];
            TestInput = [TestInput, PolynomialFeatures((i-k),:)];
        end;
        
        TrainingOutput = StockFeatures(4:(i-1),j);
        
        PredictedTables(i-(TotalDataSize-TestDataSize),j) = LinearRR(TrainingInput, TestInput, TrainingOutput,1^(-10));        
...
% Save results
save(strcat('Linear_PolynomialFeatures_PredictionTable_', stockname, ''), 'PredictedTables', 'TwitterDaysUsed');
...
xlabel('Test Day');
legend('Benchmark Model', 'Ground Truth');
...
savefig(h, strcat('Predictions_', stockname, ''));
saveas(h,strcat('Predictions_', stockname, ''),'png');
