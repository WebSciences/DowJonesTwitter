...
subplot(2,1,1);
hold on;
bar(1:50,zscore(TotalTweetsCount));
plot(1:50, zscore(StockFeatures(:,1)), 'r');
legend('Intel Tweets Count', 'Trading Volume');
xlabel('Day');

subplot(2,1,2);
hold on;
bar(1:50,zscore(TotalTweetsCount));
plot(1:50, zscore(StockFeatures(:,3)), 'r');
legend('Intel Tweets Count', 'Daily Price Change');
xlabel('Day');

ax=axes('Units','Normal','Position',[.075 .075 .85 .85],'Visible','off');
set(get(ax,'Title'),'Visible','on')
title(stockname, 'FontSize', 16, 'FontWeight', 'Bold');
...
for j = 1:4
    ...
    for i = (TotalDataSize-TestDataSize+1):TotalDataSize
        ...
        for m=1:3
            ...
            for n=0:3
                for l=(i-10):i
                    ...
                    for k=1:n
                        TrainingInput = [TrainingInput, TotalTweetsCount((4-k):(l-k-1),:)];
                        ValidationInput = [ValidationInput, TotalTweetsCount((l-k),:)];
                    end;
                    
                    TrainingOutput = StockFeatures(4:(l-1),j);
                    ValidationOutput = StockFeatures(l,j);
                    
                     %   TestInput = [StockFeatures(l,:), TotalTweetsCount(l,:)];
        ...
        for k=1:Optimaln
            TrainingInput = [TrainingInput, TotalTweetsCount((4-k):(i-k-1),:)];
            TestInput = [TestInput, TotalTweetsCount((i-k),:)];
        end;
